...
namespace CodingExamDnsLookup.Controllers
{
    ...
    {
        private readonly ILogger<DnsLookupController> logger;
        private readonly ApiClient client;

        public DnsLookupController(ILogger<DnsLookupController> nlogger, ApiClient apiClient)
        {
            logger = nlogger;
            client = apiClient;
        }
        ...
        public async Task<ActionResult<string>> GetAsync(string host = "google.com")
        {
            try
            {
                ...
                if (hostEntry != null)
                {
                    ApiResponse response = await client.GetAsync(hostEntry.HostName, ApiEndPointType.DnsLookup);

                    if (response.IsSuccessStatusCode)
                    {
                        response.Result = JObject.Parse(response.Result.ToString().Replace("query", "DnsLookup"));
